...
Currently, we are having some difficulty with deploying our third activity within `{webr}`. In that activity, we use `{brms}` for sampling to obtain our updated posterior distribution for the mean. While [`{brms}` is listed as a package which is available within `{webr}`](https://rud.is/w/webr-pkgs/) the functionality from `{brms}` builds and compiles a `Stan` program, which does not seem to be running in-browser. We would like to stick with `{brms}` functionality for these activities because it provides a convenient interface for conducting a variety of Bayesian analyses, however, we will convert this activity to make use of a Gibbs Sampler written in R so that it can be run in-browser via `{webr}`. We've filed an issue on [the `{webr}` GitHub repository](https://github.com/r-wasm/webr/issues) to make the developers aware of the issue with calling `{brms}` functionality in the `{webr}` environment.
