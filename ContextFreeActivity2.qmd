...
Now, use the updated data and your results from `summarize_beta_binomial()`, along with the `qbeta()` function to extract a new 90% credible interval for the population proportion. Use `plot_beta_ci()` to plot the credible interval as well.
...
What does this new credible interval tell you about the population parameter? Discuss what these specific interval bounds mean in practical terms for the population parameter being investigated. Additionally, think of how this interval might change if we collected more data or desired greater coverage. Under each scenario, would the interval narrow or widen? Why?
:::
