...
webr:
  ...
  packages: ["ggplot2", "dplyr", "bayesrules", "readr"]
...
There are many statistical tools which can be used to investigate population parameters. As we mentioned in [our first activity](https://agmath.github.io/IntroductoryBaysInContext/act1_bio_chytrid_v2.html), these tools fall into three broad categories:

-   Classical/Frequentist methods
-   Simulation-based methods
-   Bayesian methods

This series of activities focuses on Bayesian methods. In this second activity, we engage in extracting and interpreting a *credible interval* from a *posterior* distribution. We'll briefly review how to obtain the *posterior* distribution from the *prior* and observed data, but check back on [our first activity](https://agmath.github.io/IntroductoryBaysInContext/act1_bio_chytrid_v2.html) for full details.
...
In [Activity 1](https://agmath.github.io/IntroductoryBaysInContext/act1_bio_chytrid.html), we manually updated the prior, applying this relationship step-by-step via code, which provided transparency into the Bayesian updating process.
...
```{webr-r}
source("https://raw.githubusercontent.com/agmath/IntroductoryBayesInContext/refs/heads/main/bayesrulesfunctions.R")

print("I've made plot_beta_binomial(), summarize_beta_binomial(), and plot_beta_ci() available to you!")
```
...
summarize_beta_binomial(alpha = 1, beta = 1, y = 9, n = 13)
```
...
summarize_beta_binomial(alpha = 6, beta = 4, y = 9, n = 13)
```
...
Before we get into CIs, let's get ourselves back to where we were at the end of the last activity. As you might remember, we obtained data on a larger collection of `r new_obs` observed frogs, from which there were `r new_pos` testing positive for chytrid fungus. Run the code chunk below to read in that data.
...
print(paste0("Of the ", new_obs, 
             " newly observed frogs, there were ", 
             new_pos, " which tested positive for chytrid fungus."))
```
